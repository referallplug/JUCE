...
static AudioComponentResult getAudioComponent (AudioUnitPluginFormatHeadless& format, const PluginDescription& desc)
{
...
public:
    ...
    {
        AUInstanceParameter (AudioUnitPluginInstanceHeadless& parent,
                             UInt32 parameterID,
    ...
    explicit AudioUnitPluginInstanceHeadless (AudioComponentInstance au)
    ...
    ~AudioUnitPluginInstanceHeadless() override
    {
        ...
        if (audioUnit != nullptr)
        {
            ...
            {
                AUDeleter (AudioUnitPluginInstanceHeadless& inInstance, WaitableEvent& inEvent)
                ...
                AudioUnitPluginInstanceHeadless& auInstance;
...
private:
    ...
    {
        AUBypassParameter (AudioUnitPluginInstanceHeadless& effectToUse)
        ...
        AudioUnitPluginInstanceHeadless& parent;
                                       ...
                                       UInt64, AudioUnitParameterValue value)
    {
        ...
        static_cast<AudioUnitPluginInstanceHeadless*> (userRef)->eventCallback (*event, value);
    }
    ...
    */
    ...
    {
        return static_cast<AudioUnitPluginInstanceHeadless*> (hostRef)
    ...
    {
        return static_cast<AudioUnitPluginInstanceHeadless*> (hostRef)->renderMidiOutput (pktlist);
    }

    static OSStatus getBeatAndTempoCallback (void* hostRef, Float64* outCurrentBeat, Float64* outCurrentTempo)
    {
        return static_cast<AudioUnitPluginInstanceHeadless*> (hostRef)->getBeatAndTempo (outCurrentBeat, outCurrentTempo);
    }
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioUnitPluginInstanceHeadless)
};

//==============================================================================
template <typename TypeToCreate>
static void createAudioUnitPluginInstance (AudioUnitPluginFormatHeadless& format,
                                           const PluginDescription& desc,
                                           double rate,
                                           int blockSize,
                                           AudioPluginFormat::PluginCreationCallback callback)
{
    auto auComponentResult = getAudioComponent (format, desc);
    ...
    createAudioUnit (auComponentResult.component,
                     [rate, blockSize, origCallback = std::move (callback)] (AudioUnit audioUnit, OSStatus err)
                     {
                        if (err == noErr)
                        {
                            auto instance = std::make_unique<TypeToCreate> (audioUnit);
